...
--DROP FUNCTION fn_hist_saida_config();

CREATE OR REPLACE FUNCTION public.fn_hist_saida_config()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
	...
	AS $BODY$
		...
		BEGIN
			...
			IF (TG_OP = 'INSERT') THEN
				...
				INSERT INTO hist_saida_config
				(id_filial, dh_inc, user_inc, sentenca, mensagem)
				VALUES
				(NEW.id_filial , CURRENT_TIMESTAMP, NEW.user_inc, CURRENT_QUERY(), 'Inclusão do Registro');
			...
			ELSIF (TG_OP = 'UPDATE') THEN
				...
				FOR nome_coluna IN SELECT column_name FROM information_schema.columns WHERE table_schema = TG_TABLE_SCHEMA AND table_name = TG_TABLE_NAME LOOP
					...
					IF valor_a IS DISTINCT FROM valor_n AND nome_coluna NOT IN ('id_filial', 'dh_inc', 'dh_alt', 'user_inc', 'user_alt', 'excluido') THEN

						INSERT INTO hist_saida_config
						(id_filial, dh_inc, user_inc, campo, valor_antigo, valor_novo, sentenca, mensagem)
						VALUES
						(NEW.id_filial , CURRENT_TIMESTAMP, NEW.user_alt, coluna, valor_a, valor_n, CURRENT_QUERY(), 'Alteração do Registro');
